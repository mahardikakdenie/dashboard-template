			...
			<div class="col-span-3 bg-gray-50">
				...
				<div class="px-4 mt-4">
					<div
						...
						<div class="mb-3">
							<img src="https://dreamspos.dreamstechnologies.com/html/template/assets/img/customer/customer15.jpg "
								alt="User Avatar"
				...
				<nav class="flex-1 space-y-1 overflow-y-auto">
					...
					<TransitionGroup name="list" tag="div" class="space-y-1">
						<div v-for="item in selectedBar.child?.filter(
							(child) => child?.url !== ''
						) || []" :key="item.name"
						...
						<div v-for="childItem in selectedBar.child || []" :key="'sub-' + childItem.name">
							<div v-if="
								childItem.children &&
								childItem.children.length > 0
							">
								<div class="flex items-center justify-between w-full px-3 py-2 text-sm font-medium rounded-md cursor-pointer hover:bg-gray-100 group"
									@click="toggleDropdown(childItem.name)">
									<div class="flex items-center">
										<component v-if="childItem.icon" :is="childItem.icon" class="w-4 h-4" />
										<span class="ml-2 text-xs text-slate-500">{{ childItem.name }}</span>
									</div>
									<IconsChevron :class="{
								...
								<Transition name="slide">
									<div v-show="isOpen(childItem.name)" class="pl-6 overflow-hidden">
										<div v-for="subItem in childItem.children" :key="'sub-' + subItem.name"
